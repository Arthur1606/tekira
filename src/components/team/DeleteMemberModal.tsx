...
export function DeleteMemberModal({ member, currentUserRole }: DeleteMemberModalProps) {
  ...
  const modalContent = isOpen ? (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-md bg-[#0E1310] border border-[#232C26] rounded-2xl shadow-2xl overflow-hidden p-6 space-y-6 animate-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between border-b border-[#232C26] pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-[#F5F5F0]">Eliminar Integrante</h3>
          ...
          <button
            ...
            className="p-2 text-zinc-500 hover:text-zinc-200 hover:bg-[#141A16] rounded-lg transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <p className="text-sm text-zinc-300 leading-relaxed">
          ¿Estás seguro de eliminar a <strong className="text-[#F5F5F0]">{member.name}</strong> del equipo? Esta acción removerá su acceso de forma definitiva.
        </p>

        <form action={deleteTeamMember} className="space-y-4">
          <input type="hidden" name="member_id" value={member.id} />

          <div className="pt-3 border-t border-[#232C26] flex justify-end gap-3">
            <Button
              type="button"
              variant="secondary"
              onClick={() => setIsOpen(false)}
  ...
  return (
    <>
      <button
        ...
        className="p-2 rounded-lg bg-[#141A16] hover:bg-rose-500/10 text-zinc-400 hover:text-rose-400 border border-[#232C26] hover:border-rose-500/30 transition-colors"
